describe('template spec', () => {
  ...
  it('redirects to /items/{id} with information of the selected item on readonly view', () => {
    ...
    cy.get('textarea[name="location.address"]')
      .should('have.value', 'Palacio Regional, edificio 200, planta baja,Centro, Cuauhtemoc, Codigo Postal 06060, Ciudad de Mexico')
      .should('have.attr', 'readonly')
  });

  it('redirects to /items/{id} when clicking in an edit button with editable properties', ()=> {
    cy.visit('/items');
    cy.get('[data-testid="custom-row-1"]')
  ...
  it('opens a modal when clicking in the delete button', ()=>{
    ...
    cy.get('div[role="dialog"]').should('be.visible')
      .within(() => {
        ...
        cy.get('button[text="Cancel"]').should('be.visible')
        cy.get('button[text="Confirm"]').should('be.visible')
      })
  })
